...
(make <vm-insn-info> :name 'GSET :code 59
  ...
  :fold-lref #f
  :combined '#f
  :body '(let* ((loc)) (FETCH-OPERAND loc) (SCM_FLONUM_ENSURE_MEM VAL0) (cond ((SCM_GLOCP loc) (SCM_GLOC_SET (SCM_GLOC loc) VAL0)) ((SCM_IDENTIFIERP loc) (let* ((id::ScmIdentifier* (Scm_OutermostIdentifier (SCM_IDENTIFIER loc))) (name::ScmSymbol* (SCM_SYMBOL (-> id name))) (limit::int (SCM_VM_RUNTIME_FLAG_IS_SET vm SCM_LIMIT_MODULE_MUTATION)) (gloc::ScmGloc* (Scm_FindBinding (-> id module) name (?: limit SCM_BINDING_STAY_IN_MODULE 0)))) (when (== gloc NULL) (when limit (set! gloc (Scm_FindBinding (-> id module) name 0)) (when (!= gloc NULL) ($vm-err "can't mutate binding of %S, which is in another module" (-> id name)))) ($vm-err "symbol not defined: %S" loc)) (SCM_GLOC_SET gloc VAL0) (set! (* PC) (SCM_WORD gloc)))) (else ($vm-err "GSET: can't be here"))) INCR-PC (set! (-> vm numVals) 1) NEXT))
...
(make <vm-insn-info> :name 'TAIL-APPLY :code 149
  ...
  :fold-lref #f
  :combined '#f
  :body '(let* ((rest VAL0) (rargc::int (check_arglist_tail_for_apply vm rest)) (nargc::int (- (SCM_VM_INSN_ARG code) 2)) (proc (* (- SP nargc 1)))) (when (< rargc 0) ($vm-err "improper list not allowed: %S" rest)) (set! VAL0 proc) (post++ ARGP) (when (== rargc 0) ($goto-insn TAIL-CALL)) (set! (* (post++ SP)) rest) (DISCARD-ENV) (goto tail_apply_entry)))
...
;; #x09e  IDENTIFIERP
(make <vm-insn-info> :name 'IDENTIFIERP :code 158
  :num-params 0 :alt-num-params '()
  :operand-type 'none
  :obsoleted #f
  :fold-lref #f
  :combined '#f
  :body '($w/argr v ($result:b (SCM_IDENTIFIERP v))))

;; #x09f  NUMBERP
(make <vm-insn-info> :name 'NUMBERP :code 159
  :num-params 0 :alt-num-params '()
  :operand-type 'none
  :obsoleted #f
  :fold-lref #f
...
;; #x0a0  REALP
(make <vm-insn-info> :name 'REALP :code 160
  :num-params 0 :alt-num-params '()
  :operand-type 'none
  :obsoleted #f
  :fold-lref #f
...
(make <vm-insn-info> :name 'VEC-SET :code 169
  ...
  :fold-lref #f
  :combined '#f
  :body '(let* ((vec) (ind)) (POP-ARG ind) (POP-ARG vec) ($type-check vec SCM_VECTORP "vector") ($type-check ind SCM_INTP "fixnum") (let* ((k::int (SCM_INT_VALUE ind)) (v VAL0)) (when (or (< k 0) (>= k (SCM_VECTOR_SIZE vec))) ($vm-err "vector-set! index out of range: %d" k)) (SCM_FLONUM_ENSURE_MEM v) (set! (SCM_VECTOR_ELEMENT vec k) v) ($result SCM_UNDEFINED))))
...
(make <vm-insn-info> :name 'VEC-SETI :code 171
  ...
  :fold-lref #f
  :combined '#f
  :body '($w/argp vec ($type-check vec SCM_VECTORP "vector") (let* ((k::int (SCM_VM_INSN_ARG code)) (v VAL0)) (when (or (< k 0) (>= k (SCM_VECTOR_SIZE vec))) ($vm-err "vector-set! index out of range: %d" k)) (SCM_FLONUM_ENSURE_MEM v) (set! (SCM_VECTOR_ELEMENT vec k) v) ($result SCM_UNDEFINED))))
...
(make <vm-insn-info> :name 'PUSH-HANDLERS :code 229
  ...
  :fold-lref #f
  :combined '#f
  :body '(let* ((before) (after VAL0)) (VM-ASSERT (>= (- SP (-> vm stackBase)) 1)) (POP-ARG before) (SCM_FLONUM_ENSURE_MEM before) (SCM_FLONUM_ENSURE_MEM after) (set! (-> vm handlers) (Scm_Acons before after (-> vm handlers))) NEXT))
